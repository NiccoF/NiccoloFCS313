...
On my/our honor, Niccolo Faelnar and (no partner), this 
...
def prepare_game():
    ...
    with open("secret_words.txt", "r", encoding="ascii") as valid_secret_words:
        secrets = [word.rstrip() for word in valid_secret_words.readlines()]
    ...
    if len(sys.argv) == 1:
        ...
        random.seed(int(sys.argv[1])) # set seed to the input int
        secret_word = random.choice(secrets)
