...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for i, color in enumerate(feedback_colors):
            if color is CORRECT_COLOR:
                self.colors[guessed_word[i]] = CORRECT_COLOR
            elif color is WRONG_SPOT_COLOR:
                if self.colors[guessed_word[i]] is not CORRECT_COLOR:
                    self.colors[guessed_word[i]] = WRONG_SPOT_COLOR
            else:
                if self.colors[guessed_word[i]] is NO_COLOR:
    ...
    def __str__(self):
        ...
        """
        return_str = ""
        for letter in "qwertyuio":
            return_str += color_word(self.colors[letter], letter) + " "
        return_str += color_word(self.colors["p"], "p") + "\n "
        for letter in "asdfghjk":
            return_str += color_word(self.colors[letter], letter) + " "
        return_str += color_word(self.colors["l"], "l") + "\n   "
        for letter in "zxcvbn":
            return_str += color_word(self.colors[letter], letter) + " "
        return_str += color_word(self.colors["m"], "m")
        return return_str
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i] and letters_dict[guessed_word[i]] > 0:
            feedback[i] = CORRECT_COLOR
            letters_dict[guessed_word[i]] -= 1

    for i in range(NUM_LETTERS):
        if feedback[i] is None:
            if not guessed_word[i] in letters_dict:
                feedback[i] = NOT_IN_WORD_COLOR
            elif letters_dict[guessed_word[i]] > 0:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    return (tuple(word_families[0].feedback_colors), word_families[0].words)
